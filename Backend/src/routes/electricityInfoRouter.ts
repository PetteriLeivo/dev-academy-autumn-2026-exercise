...
router.get('/electricity', async (req: Request, res: Response): Promise<void> => {
    ...
    perus_tilastot AS (
        SELECT paiva, COALESCE(SUM(consumptionamount), 0) AS kokonaiskulutus,
        COALESCE(SUM(productionamount), 0) AS kokonaistuotanto,
        COALESCE(AVG(hourlyprice), 0) AS keskihinta
    ...
    try {
        const result = await db.query(query, [rivimaara, offset]);
        const rows = result as ElectricityDayData[];
        res.json(rows);
    } catch (err) {
